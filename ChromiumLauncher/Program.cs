...
internal static class Program
{
    ...
    private static void Main()
    {
        ...
        try
        {
            ...
            if (!File.Exists(chromiumExe))
            {
                Fail(logPath, $"Missing file: {chromiumExe}");
                return;
            }
            ...
            var chromiumStartInfo = new ProcessStartInfo
            {
                FileName = chromiumExe,
                Arguments = $"--user-data-dir=\"{profileDir}\" --proxy-server=\"socks5://127.0.0.1:1080\"",
                WorkingDirectory = baseDir,
                UseShellExecute = false
            };

            using Process? chromiumProcess = Process.Start(chromiumStartInfo);
            Log(logPath, chromiumProcess is null
                ? "Process.Start returned null for Chromium."
                : $"Chromium start requested. PID={chromiumProcess.Id}.");

            Log(logPath, "Chromium launcher finished successfully.");
        }
        catch (Exception ex)
        {
            Log(logPath, "Unhandled exception: " + ex);
            ShowError("Launcher error:" + Environment.NewLine + Environment.NewLine + ex.Message);
        }
    }
